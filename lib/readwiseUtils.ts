...
export async function getReadwiseContext(
  ...
  try {
    ...
    const embedding = await generateEmbeddings(query, 1536);

    // Query Pinecone
    console.log(
      `Querying Pinecone index: ${process.env.PINECONE_INDEX || 'reader-embeddings'}`,
    );
    console.log(`Parameters: topK=${topK}, minScore=${minScore}`);

    const response = await index.query({
      vector: (embedding as any).values ?? [],
      topK,
      includeMetadata: true,
    });

    console.log(
      `Pinecone query results: ${response.matches?.length || 0} matches found`,
    );
    ...
    if (response.matches && response.matches.length > 0) {
      ...
      response.matches.forEach((match, index) => {
        ...
        const preview =
          text.length > 60 ? `${text.substring(0, 60).trim()}...` : text.trim();

        console.log(
          `[${index + 1}] Score: ${score.toFixed(4)} | Title: "${title}"`,
        );
    ...
    console.log(
      `Filtered to ${matches.length} matches with score >= ${minScore}`,
    );
