...
function semanticChunksWithOverlap(
  ...
  const getTokens = (s: string) => {
    ...
    const tokens = paraTokenCounts.get(s);
    if (!tokens) throw new Error('Token count not found');
    return tokens;
  };
  ...
  if (buf) {
    ...
    if (lastChunkIdx >= 0) {
      const prevSentences = splitSentences(out[lastChunkIdx]).slice(
        -paraOverlap,
      );
...
async function processDocument(
  ...
  if (tagStrings.length > 0) {
    console.log(
      `Document has ${tagStrings.length} tags: ${tagStrings.join(', ')}`,
    );
  }

  // 3. Extract RAKE keywords and TF-IDF terms
  const { rakeKeywords, tfidfKeywords, boostedText } = extractKeywords(
    doc.id,
    text,
  );

  console.log(
    `Extracted ${rakeKeywords.length} RAKE keywords and ${tfidfKeywords.length} TF-IDF terms`,
  );

  // Log some of the top terms
  if (rakeKeywords.length > 0) {
    console.log(
      `Top RAKE terms: ${rakeKeywords
        .slice(0, 5)
        .map((k) => k.term)
        .join(', ')}`,
    );
  ...
  if (tfidfKeywords.length > 0) {
    console.log(
      `Top TF-IDF terms: ${tfidfKeywords
        .slice(0, 5)
        .map((k) => k.term)
        .join(', ')}`,
    );
  ...
  const dedupedTfidfTerms = [...tfidfTermsSet].filter(
    (term) => !rakeTermsSet.has(term),
  );

  console.log(
    `After deduplication: ${dedupedRakeTerms.length} RAKE terms, ${dedupedTfidfTerms.length} unique TF-IDF terms`,
  );
  ...
  const enhancedSparseVector = enhanceSparseVector(sparseVector, [
    ...rakeKeywords,
    ...tfidfKeywords,
  ]);
  ...
  const chunks = chunkText(text, 512, 2);
  ...
  for (let i = 0; i < chunkLimit; i++) {
    ...
    console.log(
      `Processing chunk ${i + 1}/${chunkLimit} (${chunkText.length} chars)`,
    );
